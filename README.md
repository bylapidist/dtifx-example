...
- `npm run dtif:build`: Executes `dtifx build generate` and then `scripts/update-build-metadata.mjs` to refresh `ops/artifacts/build/{registry,telemetry}.json` alongside the CSS/JSON outputs.
- `npm run dtif:diff`: Uses `scripts/dtif-diff-normalized.mjs` to canonicalise token source URIs before calling `dtifx diff compare`, producing JSON/Markdown evidence without metadata-only noise.
...
| `npm run dtif:diff` | `ops/artifacts/diff/*.md`, `*.json` | Review breaking or additive changes flagged in the Markdown summary according to the [diff handbook](https://github.com/bylapidist/dtifx/blob/main/docs/diff/index.md); the helper normalises token source URIs so only material value or metadata updates remain. |
